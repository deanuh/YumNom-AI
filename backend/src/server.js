...
import {
  getFoodFatSecret,
  getRestaurantFatSecret,
  fetchRestaurantFatSecret,
} from "./api/fatsecret.js";
import {
  getRestaurantTripAdvisor,
  getTAPlaceDetails,
  fetchRestaurantTANoUnsplash,
} from "./api/tripadvisor.js";
...
function startPhase(groupId, duration, phaseName, nominations) {
  ...
  io.to(groupId).emit("change_phase", {
    endsAt: groupInfo[groupId].timer.endsAt,
    phaseName: groupInfo[groupId].state,
    choices: groupInfo[groupId].choices,
  });
  ...
  groupInfo[groupId].timer.main = setTimeout(() => {
    ...
    const tally = Object.values(groupInfo[groupId].votes.polling).reduce(
      (acc, vote) => {
    ...
    if (groupInfo[groupId]?.timer.main)
      clearTimeout(groupInfo[groupId].timer.main);
    if (groupInfo[groupId]?.timer.wait)
      clearTimeout(groupInfo[groupId].timer.wait);
    ...
    switch (phaseName) {
      case "join":
        ...
        if (Object.keys(groupInfo[groupId].nominations).length === 0) {
          ...
          const uniqueRestaurantObjects = Object.values(
            nominationList.reduce((acc, nomination) => {
          ...
          console.log(groupInfo[groupId].nominations);
          groupInfo[groupId].choices = groupInfo[groupId].nominations.map(
            (obj) => obj.id
          );
          console.log(groupInfo[groupId].choices);
          io.to(groupId).emit(
            "receive_nominations",
            groupInfo[groupId].nominations
          );
    ...
    io.to(groupId).emit("change_phase", {
      endsAt: groupInfo[groupId].timer.endsAt,
      phaseName: groupInfo[groupId].state,
      results: groupInfo[groupId].votes.results,
    });
    //send event and start wait phase
    groupInfo[groupId].timer.wait = setTimeout(() => {
      if (!groupInfo[groupId]) return;
      const highestVote = Math.max(
        ...Object.values(groupInfo[groupId].votes.results)
      );
      //console.log(`results: ${JSON.stringify(groupInfo[groupId].votes.results)}`);
      const topVotes = Object.keys(groupInfo[groupId].votes.results).filter(
        (result) => groupInfo[groupId].votes.results[result] === highestVote
      );
  ...
  try {
    ...
    if (groupData) {
      ...
      const expiresAt = new Date(
        groupData.date_created.toDate().getTime() +
          groupData.secondsUntilExpiration * 1000
      );
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", async () => {
    try {
      ...
      const uidInfo = uids.reduce((acc, key) => {
        // list of the invited users information actually connected to the session.
      ...
      socket.broadcast.to(socket.groupId).emit("joined_room", {
        //everyone else is sent the user's info too
        userId: socket.uid,
        profile_picture: uidInfo[socket.uid].profile_picture,
        username: uidInfo[socket.uid].username,
      });
      console.log(
        `socket.uid: ${socket.uid}, profile_picture: ${
          uidInfo[socket.uid].profile_picture
        }, uidInfo: ${JSON.stringify(uidInfo)}`
      );
      if (!groupInfo[groupId]) return;
      socket.emit("change_phase", {
        ...
        results: groupInfo[groupId].votes.results,
        choices: groupInfo[groupId].choices,
        polling: groupInfo[groupId].votes.polling,
      });
  ...
  // --- LISTENER FOR HOST "START" BUTTON ---
  socket.on("host_start_voting", () => {
    ...
    // 2. Move everyone to the voting page
    io.to(groupId).emit("navigate_to_voting");
  });

  socket.on("send_nomination", async (nomination) => {
    if (!socket.groupId)
      return socket.emit("join_error", { message: "User not in room." });
    ...
    if (groupInfo[socket.groupId].state == "join") {
      ...
      console.log(
        `nominations: ${JSON.stringify(groupInfo[socket.groupId].nominations)}`
      );
    }
  });
